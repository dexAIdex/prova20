...
export default function Home() {
  ...
  const handleBuyWithBNB = async () => {
    ...
    try {
      ...
      // Calcola quanti token stai per comprare
      const priceFromOracle = await presaleContract.getBNBUSDPrice();
      const currentStage = await presaleContract.currentStage();

      const bnbUsdPriceOnChain = Number(priceFromOracle) / 1e8;
      const pricePerToken = 0.0001 + (Number(currentStage) - 1) * 0.000025;

      const tokensToBuy =
        (Number(bnbAmount) * bnbUsdPriceOnChain) / pricePerToken;
